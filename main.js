...
const ICONO_ESTATICO = L.divIcon({
  iconSize:[32,32],
  iconAnchor:[16,32],
  html:`<svg viewBox="0 0 384 512" width="32" height="32" fill="#dc2626">
...
async function cargarMenu(user){
  ...
  res.data.forEach(m=>{
    const [ ,nombre,archivo,icono,permiso,activo ] = m;
    if(activo!=="SI") return;
...
function iniciarMapa(){
  ...
  WATCH_ID = navigator.geolocation.watchPosition(pos=>{
    const {latitude,longitude,speed=0,heading=0}=pos.coords;
    ...
    if(!MAPA){
      ...
      MARCADOR=L.marker([latitude,longitude],{icon:ICONO_ESTATICO}).addTo(MAPA);
      CIRCULO=L.circle([latitude,longitude],{radius:80}).addTo(MAPA);
    }
